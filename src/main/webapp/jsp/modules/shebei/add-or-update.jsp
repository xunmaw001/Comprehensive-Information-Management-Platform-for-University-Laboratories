    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑实验室设备</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">实验室设备管理</li>
                        <li class="breadcrumb-item active">编辑实验室设备</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">实验室设备信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6" style="display: none">
                                        <label>审核状态</label>
                                        <input value="1" id="shenheTypesSelect" name="shenheTypes" class="form-control">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#yuanjiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('yuanjiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });


        $('#xiaojiContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('xiaojiContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
            if(window.sessionStorage.getItem('role') != '实验室人员'){//当前登录用户不为这个
                if($("#yonghuId") !=null){
    ...
    function setContent() {
        ...
        if (ruleForm.yuanjiContent != null && ruleForm.yuanjiContent != 'null' && ruleForm.yuanjiContent != '' && $("#yuanjiContentupload").length>0) {

            var yuanjiContentUeditor = UE.getEditor('yuanjiContentEditor');
            yuanjiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.yuanjiContent != null){
                    mes = ''+ ruleForm.yuanjiContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                yuanjiContentUeditor.setContent(mes);
            });
        }
        if (ruleForm.xiaojiContent != null && ruleForm.xiaojiContent != 'null' && ruleForm.xiaojiContent != '' && $("#xiaojiContentupload").length>0) {

            var xiaojiContentUeditor = UE.getEditor('xiaojiContentEditor');
            xiaojiContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.xiaojiContent != null){
                    mes = ''+ ruleForm.xiaojiContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                xiaojiContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        if($("#yuanjiContentupload").length>0) {
            var yuanjiContentEditor = UE.getEditor('yuanjiContentEditor');
            if (yuanjiContentEditor.hasContents()) {
                $('#yuanjiContent-input').attr('value', yuanjiContentEditor.getPlainTxt());
            }
        }
        if($("#xiaojiContentupload").length>0) {
            var xiaojiContentEditor = UE.getEditor('xiaojiContentEditor');
            if (xiaojiContentEditor.hasContents()) {
                $('#xiaojiContent-input').attr('value', xiaojiContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                yuanjiContent: "required",
                xiaojiContent: "required",
                shenheTypes: "required",
            },
            messages: {
                shebeiName: "设备名称不能为空",
                yonghuId: "发布实验室人员不能为空",
                insertTime: "发布时间不能为空",
                shebeiTypes: "设备类型不能为空",
                shebeiContent: "设备作用不能为空",
                yuanjiContent: "院级审核结果不能为空",
                xiaojiContent: "校级审核结果不能为空",
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#yuanjiContent").val(ruleForm.yuanjiContent);
        $("#xiaojiContent").val(ruleForm.xiaojiContent);
        $("#shenheTypesSelect").val(ruleForm.shenheTypes);
    ...
    function readonly() {
		...
		else if (window.sessionStorage.getItem('role') == '实验室人员') {
            // $(".aaaaaa").remove();
            $(".yonghu").remove();//删除当前用户的信息
        }
		else if (window.sessionStorage.getItem('role') == '校级人员') {
            // $(".aaaaaa").remove();
            $(".xiaoji").remove();//删除当前用户的信息
        }
		else if (window.sessionStorage.getItem('role') == '院级人员') {
            // $(".aaaaaa").remove();
            $(".yuanji").remove();//删除当前用户的信息
        }
        else{
            // alert("未知情况.......");
